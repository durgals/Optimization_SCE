...
if funct == testfunctn1:
    ...
    bu=np.array([2,2])
    x0=np.array([2,2])
    bestx,bestf,BESTX,BESTF,ICALL = sceua(funct,x0,args=(1,),bl=bl,bu=bu,maxn=maxn,kstop=kstop,pcento=pcento,peps=peps,ngs=ngs,iseed=iseed,iniflg=iniflg)

elif funct==testfunctn2:
    '''2
    ...
    bu=600*np.ones(2)
    x0=np.zeros(2)
    bestx,bestf,BESTX,BESTF,ICALL = sceua(funct,x0,args=(2,),bl=bl,bu=bu,maxn=maxn,kstop=kstop,pcento=pcento,peps=peps,ngs=ngs,iseed=iseed,iniflg=iniflg)    
elif funct==testfunctn6:
    '''6
...
#tendStr = tend.astimezone(mel).strftime('%a %b %d %Y %H:%M:%S %Z')
print(f"The calculation of the SCE algorithm took: Elapsed {tend-tstart}")
